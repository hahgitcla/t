// Вбудовуємо функції з local-api.js напряму
function getHash() {
  try {
...
function getFlightResult(bet) {
  try {
    ...
    return { message: 'Помилка обробки запиту' };
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    const preloaderImage = preloader.querySelector('.preloader-image');
    ...
    (async () => {
      try {
        ...
        const generatedHash = await fetchHashFromServer();
        console.log('Hash received:', generatedHash);
    ...
    console.error('Fatal error in script3.js:', error);
  }
});